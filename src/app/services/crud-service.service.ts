...
export class CrudServiceService {
  ...
  // CREATION
  async create(id, collectionName: string, objectSet, navigate: string = '') {
  ...
  async delete(collectionName, id, toastMessage = '') {
    ...
    this.afs
      ...
      .then(() => {
        loading.dismiss();
        if (toastMessage != '') this.toast(toastMessage, 'success');
      })
      .catch((error) => {
        if (toastMessage != '') this.toast(error, 'danger');
      });
  } //End of Delete Task
} // ending of class
